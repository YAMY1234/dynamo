...
class PrefillWorkerHandler(BaseWorkerHandler):
    ...
    async def generate(
        ...
        prefill_cohort_kwargs = _prefill_cohort_kwargs(inner_request.get("annotations"))
        ...
        if native_payload is not None:
            if prefill_cohort_kwargs:
                raise ValueError(
                    "explicit SGLang prefill cohorts are not supported by the native Generate path"
                )
